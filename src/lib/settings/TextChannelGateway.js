...
class TextChannelGateway extends GatewayStorage {
	 ...
	 */
	async sync(input = [...this.client.channels.cache.filter(c => c.type === `text`).map(c => `${c.guild.id}.${c.id}`), ...[]]) {
		if (Array.isArray(input)) {
			...
			for (const guild of this.client.guilds.cache.values()) {
				for (const channel of guild.channels.cache.filter((channel) => channel.type === `text`).values()) if (channel.settings._existsInDB !== true) channel.settings._existsInDB = false;
			}
			return this;
		}
